...
import { createNtCuratorFrameworkAndConnect, createRandomizedPath } from "../../tests/util";
...
describe("ReadLock", () => {
  ...
  describe("tryAcquire", () => {
    ...
    it("should return false if recursive creation of non-leaf nodes failed", async () => {
      jest.spyOn(PathUtility.prototype, "ensurePathExists").mockRejectedValue(new Error());
      const ntCuratorFramework = await createNtCuratorFrameworkAndConnect();
      const path = createRandomizedPath();
      const readLock = new ReadLock(ntCuratorFramework, path);
    ...
    it("should return false if the creation of the leaf read lock node failed", async () => {
      ...
      const ntCuratorFramework = await createNtCuratorFrameworkAndConnect();
      jest.spyOn(ntCuratorFramework.client, "create").mockImplementation((path, p1, p2, p3, p4) => {
    ...
    it("should return false if getting the children of level before leaf read lock node failed", async () => {
      ...
      jest.spyOn(ntCuratorFramework.client, "getChildren").mockImplementation((path, p1, p2) => {
        const stat = {
          czxid: 0,
          mzxid: 0,
          ctime: 0,
        ...
        if (typeof p1 === "function") return (p1 as Function)(new Error(), [], stat);
      });
      const path = createRandomizedPath();
      const readLock = new ReadLock(ntCuratorFramework, path);
      const { acquired, error } = await readLock.tryAcquire();
    ...
    it("should return false if there exist a sibling node that is a write node and has a smaller sequence number", async () => {
      ...
      jest.spyOn(ntCuratorFramework.client, "getChildren").mockImplementation((path, p1, p2) => {
        ...
        if (typeof p2 === "function") return (p2 as Function)(null, children, stat);
        if (typeof p1 === "function") return (p1 as Function)(null, children, stat);
      });
      jest.spyOn(ntCuratorFramework.client, "remove").mockImplementation((path, p1, p2) => {
        if (typeof p1 === "function") return (p1 as Function)(null);
        if (typeof p2 === "function") return (p2 as Function)(null);
      });
      const path = createRandomizedPath();
      const readLock = new ReadLock(ntCuratorFramework, path);
      await expect(readLock.tryAcquire()).resolves.toStrictEqual({ acquired: false, error: null });
    });
    ...
    it("should return true if there exist a sibling node that is a write node but has a bigger sequence number", async () => {
      ...
      const path = createRandomizedPath();
      const readLock = new ReadLock(ntCuratorFramework, path);
      await expect(readLock.tryAcquire()).resolves.toStrictEqual({ acquired: true, error: null })
    });
  ...
  describe("release", () => {
    ...
    it("should return true if the read node deletion failed as there's no node found", async () => {
      ...
      await expect(readLock.release()).resolves.toStrictEqual({ released: true, error: null })
    });
