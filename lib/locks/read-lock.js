...
class ReadLock {
        ...
        const acquireLock = async () => {
            try {
                if (client.getState().code !== node_zookeeper_client_1.State.SYNC_CONNECTED.code) {
                    return { acquired: false, error: new lock_error_1.LockError(lock_error_1.LockErrorCodes.NOT_CONNECTED, "Failed to acquire the read lock because of connection issues.") };
                }
                ...
                return { acquired: false, error: new lock_error_1.LockError(lock_error_1.LockErrorCodes.UNEXPECTED_ERROR, "An unexpected error occurred while trying to acquire the read lock.") };
            }
            finally {
    ...
    async release() {
        try {
            const client = this.ntCuratorFramework.getClient();
            if (client.getState().code !== node_zookeeper_client_1.State.SYNC_CONNECTED.code) {
                return { released: false, error: new lock_error_1.LockError(lock_error_1.LockErrorCodes.NOT_CONNECTED, "Failed to release the read lock because of connection issues.") };
            }
            try {
                await new Promise((res, rej) => {
            ...
            return { released: false, error: new lock_error_1.LockError(lock_error_1.LockErrorCodes.UNEXPECTED_ERROR, "An unexpected error occurred while trying to release the read lock.") };
        }
    }
}
